...
con métricas de entropía, divergencia KL, información mutiva y ganancia
de información.
...
class InformationTheoryScorer:
    ...
    def score_relevance(self, candidate_probs: list[float], reference_probs: list[float]) -> float:
        ...
        if kl == float('inf'):
            return 0.0
        # Normalizar: KL máxima posible es log2(n)
    ...
    def score_batch(self, candidates: list[dict[str, Any]], 
                    reference: dict[str, Any] | None = None) -> list[dict[str, float]]:
    ...
    Returns:
        ...
        for cand in candidates:
            ...
            ref_probs_n = normalize_to_probs(ref_probs) if ref_probs else [1.0/len(cand_probs)] * len(cand_probs)
