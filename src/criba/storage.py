...
class Storage:
    ...
    def save(self, query: str, packet: dict, config: dict) -> str:
        ident=packet["activation_id"]; now=packet["timestamp"]
    ...
    def record_decision(self, session_id:str, status:str, evidence:list|dict, note:str="") -> dict:
        if status not in VALID_DECISIONS: raise ValueError("Estado de decisión inválido.")
        entry={"id":str(uuid.uuid4()),"session_id":session_id,"timestamp":datetime.now(timezone.utc).isoformat(),"status":status,"evidence":evidence,"note":note}
        with self.connect() as con:
