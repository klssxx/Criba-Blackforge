...
# Uses the project-managed uv environment (self-contained, reproducible).
$ErrorActionPreference = 'Stop'
$root = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path
$UV = (Get-Command uv -CommandType Application -ErrorAction SilentlyContinue).Source
if (-not $UV) {
    Write-Error 'uv not found. Install uv 0.11.28 and run "uv sync --all-extras --locked".'
    exit 1
}
$spec = Join-Path $root 'CRIBA-Blackforge.spec'
Push-Location $root
try {
    & $UV run --locked --all-extras python -m PyInstaller --noconfirm --clean $spec
