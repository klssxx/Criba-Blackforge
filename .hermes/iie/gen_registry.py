...
IMPLEMENTATION_OVERRIDES = {
    ...
    "T091": {
        "implementation": "criba.intelligence.graph.link_prediction.LinkPredictionInterface.predict",
        "input_contracts": ["graph store con entidades/relaciones", "source: str", "limit: int = 10"],
        "output_contracts": ["list[dict] enlaces ausentes rankeados"],
        "tests": ["test_predict_returns_ranked_absent_links_by_common_neighbors",
                  "test_predict_excludes_existing_links",
                  "test_predict_rejects_negative_limit_and_accepts_zero"],
    },
    "T094": {
        "implementation": "criba.intelligence.graph.communities.CommunityDetector.detect",
        "input_contracts": ["graph store", "entity_ids: Iterable[str] | None = None"],
        "output_contracts": ["list[list[str]] comunidades"],
        "tests": ["test_detect_returns_sorted_weak_communities",
                  "test_detect_restricts_communities_to_requested_subset"],
    },
    "T095": {
        "implementation": "criba.intelligence.graph.bridges.BridgeNodeAnalyzer.articulation_points",
        "input_contracts": ["graph store", "entity_ids: Iterable[str] | None = None"],
        "output_contracts": ["list[str] puntos de articulacion"],
        "tests": ["test_articulation_points_are_sorted_and_exclude_leaves",
                  "test_articulation_points_respect_requested_subset"],
    },
    "T129": {
...
CANON_VERSION = "2026-09-08.3"  # bump when reg_parts/*.txt or overrides change
